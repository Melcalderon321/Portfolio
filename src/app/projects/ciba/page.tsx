...
import { ArrowLeft, Target, Brain, CheckCircle2 } from 'lucide-react'
import Link from 'next/link'

export default function CibaCaseStudy() {
    const { t, lang } = useI18n()

    const data = t.product_optimization.cases.find((c: any) => c.id === 'ciba')
    if (!data) return null
    ...
    return (
        <main style={{ minHeight: '100vh', background: 'var(--background)', color: 'white' }}>
            ...
            <section className="container" style={{ padding: '4rem 1.5rem 8rem' }}>
                {/* 1. Header and Context/Objective (900px) */}
                            ...
                            {data.sections.objective && (
                                <div>
                                    <h3 style={{ fontSize: '1.25rem', color: 'var(--neon-pink)', marginBottom: '0.75rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <Target size={22} /> {data.sections.objective.title}
                                    </h3>
                                    <p style={{ opacity: 0.8, fontSize: '1.05rem', lineHeight: '1.7' }}>
                                        {data.sections.objective.text}
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
                        ...
                        {data.sections.investigation && (
                            <div>
                                ...
                                {data.sections.investigation.bullets && (
                                    <ul style={{ paddingLeft: '1.2rem', opacity: 0.8, fontSize: '1.05rem', lineHeight: '1.7' }}>
                                        {data.sections.investigation.bullets.map((b: string, bIdx: number) => (
                            ...
                            {data.sections.solution && (
                                <div>
                                    <h3 style={{ fontSize: '1.25rem', color: 'white', marginBottom: '0.75rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <Brain size={22} color="var(--neon-pink)" /> {data.sections.solution.title}
                                    </h3>
                                    <p style={{ opacity: 0.8, fontSize: '1.05rem', lineHeight: '1.7', marginBottom: '1rem' }}>
                                        {data.sections.solution.text}
                                    </p>
                                    {data.sections.solution.bullets && (
                                        <ul style={{ paddingLeft: '1.2rem', opacity: 0.8, fontSize: '1.05rem', lineHeight: '1.7' }}>
                                            {data.sections.solution.bullets.map((b: string, bIdx: number) => (
                                                <li key={bIdx} style={{ marginBottom: '0.5rem' }}>{b}</li>
                                            ))}
                                        </ul>
                                    )}
                        ...
                        {data.sections.impact && (
                            <div style={{ padding: '0.5rem 0' }}>
                                <h3 style={{ fontSize: '1.25rem', color: 'var(--neon-pink)', marginBottom: '0.75rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <CheckCircle2 size={22} /> {data.sections.impact.title}
                                </h3>
                                <p style={{ opacity: 0.9, fontSize: '1.05rem', lineHeight: '1.7', color: 'white' }}>
                                    {data.sections.impact.text}
                                </p>
                            </div>
                        )}
                    </div>
